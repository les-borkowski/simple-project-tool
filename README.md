...
| `CREDENTIAL_ENCRYPTION_KEY` | *(empty)* | comma-separated Fernet keys, **newest first**; required before any user can save a personal credential — an empty value means `503 CREDENTIAL_STORAGE_UNAVAILABLE` on save. Generate one from `backend/` with `uv run python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"` (`uv run`, not plain `python` — `cryptography` is a backend dependency and macOS has no bare `python`). A dedicated key, deliberately not derived from `SECRET_KEY`, so rotating one never invalidates the other. See rotation below |
