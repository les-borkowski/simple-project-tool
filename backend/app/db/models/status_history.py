...
from sqlalchemy import ForeignKey, CheckConstraint, Index, func
...
class StatusHistory(Base):
    ...
    story_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("stories.id", ondelete="CASCADE"), nullable=True)
    task_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("tasks.id", ondelete="CASCADE"), nullable=True)
    from_status: Mapped[Optional[StatusEnum]] = mapped_column(nullable=True)
