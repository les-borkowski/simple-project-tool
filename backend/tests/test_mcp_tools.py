...
async def mcp_client(api_client: AsyncClient, api_db: AsyncSession, manager_user: User):
    """An SPTClient hitting the real ASGI app in-process, authenticated with a real,
    read-only-scoped API key minted through the production hashing path.
    ...
    key = await config_service.create_api_key(
        APIKeyCreate(
            label="mcp-test-key",
            scopes=["read:projects", "read:stories", "read:tasks", "read:comments"],
        ),
        manager_user,
        api_db,
    )
    config = MCPConfig(api_url="http://test", api_key=key.key, locale="en-GB", timeout=30.0)
    client = SPTClient(config, transport=httpx.ASGITransport(app=app))
    yield client
    await client.aclose()
